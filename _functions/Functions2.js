...
async function ApplyCompRace(newRace, prefix, sCompType, bIsRaceFld, oldValue) {
	...
	if (aCrea.typeFound === "race") {// do the following if a race was found
		...
		await SetCreatureType(prefix, [strRaceEntry, strRaceEntry, sCompExplStr], aCrea.type, aCrea.subtype);
...
async function ApplyWildshape(fldName, value, oldValue) {
	...
	if (newCrea === oldCrea || !newCrea || !wasm_character.get_level() || !wasm_character.get_ability("Int")|| !wasm_character.get_ability("Wis")|| !wasm_character.get_ability("Cha")) {
	...
	// Size
	await SetCreatureSize([prefix, Fld], oWS.name, oWS.size);

	// Creature type
	await SetCreatureType([prefix, Fld], oWS.name, oWS.type, oWS.subtype);
...
function CalcWildShapeInfo(fldName) {
	var prefix = getTemplPre(fldName, "WSfront", true);
	if (!prefix) return;
	var theField = fldName.replace(prefix, "");
	return What(theField);
}

// When changed, set the value to the primary (i.e. non-spawned template) [field validation]
function ValidateWildShapeInfo(fldName, fldValue) {
	var prefix = getTemplPre(fldName, "WSfront", true);
	if (!prefix) return;
	var theField = fldName.replace(prefix, "");
	var theValue = What(theField);
	if (fldValue !== theValue) Value(theField, fldValue);
}
...
async function SetCreatureType(prefix, sName, aTypes, aSubtypes) {
	var sNamePl = sName;
	var sNameCompanion = "";
	if (isArray(sName)) {
	...
	var sCreaType = !isArray(aTypes) ? aTypes :
		await AskUserOptions(oDialogTxt.title, oDialogTxt.description, aTypes, "radio", true);
	var sCreaSubtype = !aSubtypes ? "" : !isArray(aSubtypes) ? aSubtypes :
		await AskUserOptions(oDialogTxt.title.replace(), oDialogTxt.description, aSubtypes, "radio", true);
