...
/* TESTING - old way below
			...
			for (var i = 0; i < abiArr.length; i++) {
				...
				var thisMod = !abiArr[i] ? 0 : wasm_character.get_ability(AbilityScores.abbreviations[abiArr[i] - 1]);
...
function getAbiModValue(ability, prefix, wildshapeNo, bReturnScore) {
	...
	if (!prefix) {
		mod = bReturnScore ? wasm_character.get_ability(abi) : wasm_character.get_ability_modifier(abi);
	} else if (wildshapeNo) {
