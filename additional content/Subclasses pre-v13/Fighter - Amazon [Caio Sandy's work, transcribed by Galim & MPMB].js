...
SourceList["CS:A"] = {
	...
	url : "https://www.dmsguild.com/product/171865/Fighter-archetype-Amazon",
	date : "2016/03/11"
};
...
AddSubClass("fighter", "amazon", {
	...
	features : {
		...
		"subclassfeature7" : {
			...
			source : ["CS:A", 1],
			minlevel : 7,
			description : desc('Choose an Amazon Skill using the "Choose Feature" button above'),
			...
			"fighting style: mariner" : {
				...
				eval : "await AddACMisc(1, \"Mariner Fighting Style\", \"When not wearing heavy armor or using a shield, the class feature Mariner Fighting Style gives a +1 bonus to AC\", \"ACshield || tDoc.getField('Heavy Armor').isBoxChecked(0)\")",
				removeeval : "await AddACMisc(0, \"Mariner Fighting Style\", \"When not wearing heavy armor or using a shield, the class feature Mariner Fighting Style gives a +1 bonus to AC\")"
			},
			...
			"escape the horde" : {
				...
				source : ["CS:A", 1],
				description: desc("Opportunity attacks made against me are made with disadvantage")
			},
			"fend" : {
				name : "Amazon Skill: Fend",
				source : ["CS:A", 1],
				description: desc("As an action, I can make an attack with my spear or glaive vs. all within my reach"),
				action : [["action", ""]]
			},
			"strafe" : {
			...
			"uncanny dodge" : {
				...
				source : ["CS:A", 1],
				description: desc("As a reaction when hit by an attacker that I can see, I can halve the attack's damage"),
				action : [["reaction", ""]]
			},
		},
		"subclassfeature15" : {
		...
		"subclassfeature15.1" : {
			...
			source : ["CS:A", 1],
			minlevel : 15,
			description: desc("I regain one superiority die if I have no more remaining when I roll initiative"), //changed to be the same as the battle master
		},
		"subclassfeature18" : {
